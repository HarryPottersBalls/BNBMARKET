...
// Get LMSR probabilities for a market with enhanced dynamics
app.get('/api/markets/:id/probabilities', async (req, res) => {
  try {
    const { id } = req.params;
    const { lookback = 50 } = req.query;

    // Get market with detailed information
    const marketResult = await queryDatabase(`
      SELECT
        m.*,
        COALESCE(m.metadata->>'liquidity_param', '10') as liquidity_param,
        COALESCE(m.metadata->>'use_lmsr', 'true') as use_lmsr
    FROM markets m
    WHERE id = $1`, [id]);
    ...
    // Get recent bets with timestamp for more granular analysis
    const betsResult = await queryDatabase(`
      SELECT
        option_id,
        amount,
        created_at,
        EXTRACT(EPOCH FROM (created_at - (SELECT MIN(created_at) FROM bets WHERE market_id = $1)) / 3600) as hours_since_first_bet
      FROM bets
      WHERE market_id = $1 AND status = 'confirmed'
      ORDER BY created_at DESC
      LIMIT $2
    ...
    // Calculate volumes with time-weighted decay
    const outcomeVolumes = Array(numOutcomes).fill(0);
    const liquidity = parseFloat(market.liquidity_param);

    bets.forEach(bet => {
      if (bet.option_id < numOutcomes) {
        // Time decay factor: more recent bets have higher weight
        const timeDecay = Math.exp(-bet.hours_since_first_bet / 24);
        outcomeVolumes[bet.option_id] += parseFloat(bet.amount || 0) * timeDecay;
      }
    });

    // Enhanced probability calculation
    let probabilities;
    if (market.use_lmsr === 'true') {
      probabilities = calculateLMSRProbabilities(outcomeVolumes, liquidity);
    } else {
      // Fallback to simple volume-based probabilities
      const totalVolume = outcomeVolumes.reduce((sum, vol) => sum + vol, 0);
      probabilities = outcomeVolumes.map(vol =>
        totalVolume > 0 ? vol / totalVolume : 1 / numOutcomes
      );
    }

    // Optional: Admin-provided odds override
    const adminOdds = market.metadata?.admin_odds;
    if (Array.isArray(adminOdds) && adminOdds.length === numOutcomes) {
      const totalAdminOdds = adminOdds.reduce((sum, odd) => sum + odd, 0);
      probabilities = adminOdds.map(odd => odd / totalAdminOdds);
    }
    ...
    console.error('Market Probabilities API Error:', error);
    res.status(500).json({
